...
admin = "1188443395" # Pastikan ID admin Anda benar

# --- Konfigurasi Mail.tm ---
MAILTM_API_BASE = "https://api.mail.tm"

# --- Inisialisasi File Admin (Dilakukan sekali saat startup) ---
# Dibuat menjadi fungsi untuk memastikan urutan yang benar
def initialize_files():
    if not os.path.exists("admin"):
        os.makedirs("admin")

    # Inisialisasi folder users
    users_directory = "admin/users/"
    if not os.path.exists(users_directory):
        os.makedirs(users_directory)
        
    # Inisialisasi file statistik
    for total_file in ["admin/total.txt", "admin/mail.txt"]:
        if not os.path.exists(total_file):
            with open(total_file, 'w') as f:
                f.write("0")
    print("Struktur direktori admin berhasil diverifikasi.")

# Panggil inisialisasi di awal script
initialize_files()
...
def get_available_domain():
    try:
        ...
        if domains:
            # Mengambil domain pertama yang isActive=True (Mail.tm API biasanya mengembalikan yang aktif)
...
def start_bot_polling():
    ...
    while True:
        try:
            ...
            # Interval=0 agar polling secepat mungkin
...
if __name__ == '__main__':
    ...
    # 2. Jalankan Flask di Thread Utama (blocking)
    print("Flask Server (Keep Alive) dimulai di 0.0.0.0:8080...")
    run_flask_app()
